...
Java_io_canccode_aca_LlamaBridge_generateNative(
    ...
    std::vector<llama_token> tokens = llama_tokenize(g_ctx, c_prompt, true);
    ...
    // Clear any previous content in sequence 0 (important for single-turn / stateless usage)
    llama_memory_seq_rm(g_ctx, 0, -1, -1);   // ← FIXED LINE
    ...
    for (int i = 0; i < maxTokens; i++) {
        ...
        next.token[0]     = tok;
        next.pos[0]       = llama_get_kv_cache_used_cells(g_ctx);   // note: function still exists (2025)
        next.seq_id[0][0] = 0;
        next.n_seq_id[0]  = 1;
        next.logits[0]    = 1;
